...
Currently supported are builds for Linux on the x64 architecture. These scripts also run without problems on [Ubuntu on WSL](https://ubuntu.com/wsl), which means that you can them on a Windows 10 machine.

This repository used to contain scripts for building on Windows as well, but they stopped working. You can find the last working build result [here](https://github.com/xarantolus/subresource_filter_tools/releases/tag/last_windows). It might be outdated.

You should run this on a SSD of sufficient size (because you'll end up with ~700k files that are ~22GB in size).

### Scripts
The following scripts are in the [`linux`](linux) subdirectory:
